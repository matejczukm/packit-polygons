...
def is_placement_valid(board, triangle):
    ...
    '''
    assert isinstance(board, np.ndarray) and isinstance(triangle, np.ndarray), f'Invalid data types. board: {type(board)}, triangle: {type(triangle)}'
...
def get_possible_placements(board, k, turn):
    ...
    n = board.shape[0] 
    empty_cells_num = n**2 - board.astype(bool).sum() # TODO: decide whether we want to store only ones in board or turn numbers
    result = []
...
def place_polygon(board, polygon):
    ...
    '''
    assert isinstance(board, np.ndarray) and isinstance(polygon, np.ndarray), f'Invalid data types. board: {type(board)}, triangle: {type(polygon)}'
